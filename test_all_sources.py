...
def test_source_sync(source_name, url, format_choice="flac", use_cookies=False):
    ...
    # Logic from controller: use ytsearch fallback if direct download not likely to work
    track_url = track_data.get('url')
    if not track_url or any(x in track_url for x in ['spotify', 'tidal', 'beatport']):
...
if __name__ == "__main__":
    tests = [
        ("Beatport (Lib)", "https://www.beatport.com/library", "flac", True),
        ("SoundCloud (Alias)", "https://soundcloud.com/you/sets", "mp3", True),
        ("Bandcamp (Album)", "https://triprecords.bandcamp.com/album/trip-2022-vol-1", "flac", False),
        ("Tidal (Mix)", "https://tidal.com/browse/mix/01681465ba284c97a2ff7c7dfdc0f2", "flac", True),
        ("Spotify (PL)", "https://open.spotify.com/playlist/7Cj1WMVuO16YhuFToVtrtN", "mp3", True),
        ("Text-YOLO", "Noisia - Dead Limit", "flac", False)
    ]
